<script>
	...
	onMount(async () => {
		try {
			// Fetch the first/latest event (or use specific criteria)
			...
			console.log('Fetched event:', fetchedData);

			if (fetchedData) {
				event = {
					...fetchedData,
					featuredImage: fetchedData.featuredImage
						? urlFor(fetchedData.featuredImage).width(1200).url()
						: ''
				};
				// Convert PortableText → HTML
				if (fetchedData.description) {
					descriptionHTML = toHTML(fetchedData.description, {
						components: {
							types: {
								image: ({ value }) =>
				...
				startCountdown(new Date(fetchedData.startDate));
			}
		} catch (error) {
	...
	// 	// For colored text spans (your editor color plugin)
	// 	marks: {
	// 		color: ({ children, value }) => {
...
{#if loading}
	<div class="flex min-h-screen items-center justify-center">
		<p class="text-2xl">Loading event...</p>
	</div>
{:else if event}
	<section
	...
	>
		...
		<div class="relative z-10 mx-auto w-full max-w-[1300px] items-start px-6 text-left">
			...
			<div class="flex gap-2 font-[900]">
				...
				<p>></p>
				<p class="text-[#306b86] underline">Event ( {event.title || 'TBA'} )</p>
			</div>
		</div>
	</section>
	<section class="mx-auto mb-30 grid max-w-[1300px] grid-cols-1 gap-10 px-5 py-20 xl:grid-cols-3">
		...
		<div class="bg-[#1a1c1b] xl:col-span-2">
			...
			<div class="group relative cursor-pointer" on:click={() => openModal(event.featuredImage)}>
		...
		<!-- Sidebar -->
		<div class="bg-[#1a1c1b] p-10 xl:col-span-1 max-h-[1000px]">
...
{:else}
	<p class="py-20 text-center text-2xl">No event available.</p>
{/if}
