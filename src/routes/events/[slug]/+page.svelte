	...
	async function handleShare() {
		...
		if (navigator.share) {
			try {
				...
				shareActive = true;
				setTimeout(() => (shareActive = false), 2000);
			} catch (err) {
			...
			// Fallback: toggle custom share menu for desktop
...
>
	...
	<div class="relative z-10 mx-auto w-full max-w-[1300px] items-start px-6 text-left">
		...
		<div class="flex gap-2 font-[900]">
			...
			<p>></p>
			<a href="/events" class="hover:text-[#306b86]">Events</a>
			<p>></p>
	...
	<section class="mx-auto mb-30 grid max-w-[1300px] grid-cols-1 gap-10 px-5 py-20 xl:grid-cols-3">
		...
		<div class="max-h-[1000px] bg-[#1a1c1b] p-10 xl:col-span-1">
			<div class="flex items-center justify-between">
				...
				{shareActive ? 'fill-blue-500 stroke-blue-500' : 'fill-transparent stroke-gray-400'}"
									d="M18 8c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3c0 .199.02.393.057.581l-5.447 3.63A2.985 2.985 0 006 8c-1.657 0-3 1.343-3 3s1.343 3 3 3a2.985 2.985 0 003.61-1.211l5.447 3.63A2.973 2.973 0 0015 17c0 1.657 1.343 3 3 3s3-1.343 3-3-1.343-3-3-3a2.985 2.985 0 00-3.61 1.211l-5.447-3.63A2.973 2.973 0 009 11c0-.199-.02-.393-.057-.581l5.447-3.63A2.985 2.985 0 0018 8z"
								/>
							</svg>
						</button>
						...
						{#if showShareMenu}
							<div
								class="absolute top-full right-0 z-50 mt-2 w-64 rounded-lg border border-gray-700 bg-[#1a1c1b] p-4 shadow-xl"
							>
								<p class="mb-3 text-sm font-semibold text-white">Share this event</p>

								<!-- Social Share Buttons -->
								<div class="mb-3 grid grid-cols-5 gap-2">
									<button
										on:click={() => shareToSocial('twitter')}
										class="flex flex-col items-center gap-1 rounded-lg p-2 transition hover:bg-gray-800"
									...
									>
										<svg class="h-6 w-6 text-[#4267B2]" fill="currentColor" viewBox="0 0 24 24">
											<path
									...
									>
										<svg class="h-6 w-6 text-[#0077b5]" fill="currentColor" viewBox="0 0 24 24">
											<path
									...
									>
										<svg class="h-6 w-6 text-[#0088cc]" fill="currentColor" viewBox="0 0 24 24">
											<path
								...
								<button
									on:click={copyLink}
									class="w-full rounded-lg border border-gray-700 bg-gray-800 px-4 py-2 text-sm font-semibold transition hover:bg-gray-700 {copySuccess
										? 'text-green-400'
										: 'text-white'}"
								>
